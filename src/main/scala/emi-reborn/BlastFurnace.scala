...
class EmiBlastFurnaceRecipe[R <: RebornRecipe](recipe: R) extends AbstractEnergyConsumingMachineRecipe[R], AbstractMachineRecipe[R](recipe, EmiBlastFurnaceRecipe.CATEGORY) {
  override def addWidgets(holder: WidgetHolder): Unit = 
    holder.add(SlotWidget(getInput(0), 55 - 17, 35 - 19).output(false))
    holder.add(SlotWidget(getInput(1), 55 - 17, 55 - 19).output(false))
    holder.add(SlotWidget(getOutput(0), 101 -17, 45 - 19).output(true))
    holder.add(SlotWidget(getOutput(1), 101 + 20 - 17, 45 - 19).output(true))
    ...
    holder.addText(neededHeat, getDisplayWidth() - 5, 5, 0xFF040404, false)
