...
class EmiDistillationTowerRecipe[R <: RebornRecipe](recipe: R) extends AbstractEnergyConsumingMachineRecipe[R], AbstractMachineRecipe[R](recipe, EmiDistillationTowerRecipe.CATEGORY) {
  override def addWidgets(holder: WidgetHolder): Unit = 
    holder.add(SlotWidget(getInput(0), 55 - 23, 35 - 19).output(false))
    holder.add(SlotWidget(getInput(1), 55 - 23, 55 - 19).output(false))
    holder.add(SlotWidget(EmiStack.EMPTY, 101 - 23 - 5, 45 - 19 - 5).customBackground(null, 0, 0, 66, 26))
    holder.add(SlotWidget(getOutput(0), 101 - 23, 45 - 19).drawBack(false).recipeContext(this))
    holder.add(SlotWidget(getOutput(1), 101 + 20 - 23, 45 - 19).drawBack(false).recipeContext(this))
    holder.add(SlotWidget(getOutput(2), 101 + 40 - 23, 45 - 19).drawBack(false).recipeContext(this))
    holder.add(RebornProgressWidget(76 - 23, 48 - 19, time * 50, GuiBuilder.ProgressDirection.RIGHT))

    holder.addText(TranslatableText("techreborn.jei.recipe.processing.time.3", DecimalFormat("###.##").format(time.toFloat / 20)).asOrderedText(), getDisplayWidth() - 5, 5, 0xFF404040, false)
}
