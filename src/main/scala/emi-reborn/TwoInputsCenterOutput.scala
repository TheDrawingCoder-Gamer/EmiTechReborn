...
class EmiTwoInputsCenterOutputRecipe[R <: RebornRecipe](recipe: R, category: EmiRecipeCategory) extends AbstractEnergyConsumingMachineRecipe[R], AbstractMachineRecipe[R](recipe, category) {
  override def addWidgets(widgets: WidgetHolder): Unit =
    ...
    widgets.add(SlotWidget(getInput(0), 29, 26).output(false))
    widgets.add(SlotWidget(getInput(1), 29 + 92, 26).output(false))
    widgets.add(SlotWidget(EmiStack.EMPTY, 29 + 46, 26).output(true))
    widgets.add(SlotWidget(getOutput(0), 29 + 46, 26).drawBack(false).output(true).recipeContext(this))
